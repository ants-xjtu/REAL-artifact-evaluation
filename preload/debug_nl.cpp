...
static const char *nlmsg_type2str(uint16_t type)
{
	switch (type) {
	/* Generic */
	case NLMSG_NOOP:
...
static const char *af_type2str(int type)
{
	switch (type) {
	case AF_UNSPEC:
		return "AF_UNSPEC";
...
static const char *ifi_type2str(int type)
{
	switch (type) {
	case ARPHRD_ETHER:
		return "ETHER";
...
static const char *ifla_pdr_type2str(int type)
{
	switch (type) {
...
static const char *ifla_info_type2str(int type)
{
	switch (type) {
...
static const char *rta_type2str(int type)
{
	switch (type) {
	case IFLA_UNSPEC:
		return "UNSPEC";
...
static const char *rtm_type2str(int type)
{
	switch (type) {
	case RTN_UNSPEC:
		return "UNSPEC";
...
static const char *rtm_protocol2str(int type)
{
	switch (type) {
	case RTPROT_UNSPEC:
		return "UNSPEC";
...
static const char *rtm_scope2str(int type)
{
	switch (type) {
...
static const char *rtm_rta2str(int type)
{
	switch (type) {
	case RTA_UNSPEC:
		return "UNSPEC";
...
static const char *neigh_rta2str(int type)
{
	switch (type) {
	case NDA_UNSPEC:
		return "UNSPEC";
...
static const char *ifa_rta2str(int type)
{
	switch (type) {
	case IFA_UNSPEC:
		return "UNSPEC";
...
static const char *nhm_rta2str(int type)
{
	switch (type) {
	case NHA_UNSPEC:
		return "UNSPEC";
...
static const char *frh_rta2str(int type)
{
	switch (type) {
	case FRA_DST:
		return "DST";
...
static const char *frh_action2str(uint8_t action)
{
	switch (action) {
	case FR_ACT_TO_TBL:
		return "TO_TBL";
...
static size_t strlcat_static(char *__restrict dest,
	       const char *__restrict src, size_t destsize)
{
	size_t src_length = strlen(src);

	/* Our implementation strlcat_static supports dest == NULL if size == 0
			      ...
			      char *buf, size_t buflen)
{
	...
	if (buf[0])
		strlcat_static(buf, ",", buflen);

	strlcat_static(buf, flagstr, buflen);
}

static const char *nlmsg_flags2str(uint16_t flags, char *buf, size_t buflen)
{
	const char *bufp = buf;
...
static const char *if_flags2str(uint32_t flags, char *buf, size_t buflen)
{
	const char *bufp = buf;
...
static const char *rtm_flags2str(uint32_t flags, char *buf, size_t buflen)
{
	const char *bufp = buf;
...
static const char *neigh_state2str(uint32_t flags, char *buf, size_t buflen)
{
	const char *bufp = buf;
...
static const char *neigh_flags2str(uint32_t flags, char *buf, size_t buflen)
{
	const char *bufp = buf;
...
static const char *ifa_flags2str(uint32_t flags, char *buf, size_t buflen)
{
	const char *bufp = buf;
...
static const char *nh_flags2str(uint32_t flags, char *buf, size_t buflen)
{
	const char *bufp = buf;
...
next_rta:
	...
	case IFLA_PROTO_DOWN:
		...
		break;
	case IFLA_ADDRESS:
		datap = (uint8_t *)RTA_DATA(rta);
		dbuf[0] = 0;
		for (it = 0; it < plen; it++) {
			snprintf(bytestr, sizeof(bytestr), "%02X:", *datap);
			strlcat_static(dbuf, bytestr, sizeof(dbuf));
			datap++;
		}
		/* Remove trailing ':'. */
		if (dbuf[0])
	...
	case IFLA_LINKINFO:
		nllink_linkinfo_dump((struct rtattr *)RTA_DATA(rta), plen);
		break;
...
next_rta:
	...
	case RTA_PREFSRC:
		...
		case sizeof(struct in_addr):
			zlog_debug("      %xI4",
				   ((struct in_addr *)RTA_DATA(rta))->s_addr);
			break;
		case sizeof(struct in6_addr):
			zlog_debug("      %x%x%x%xI6",
				   ((struct in6_addr *)RTA_DATA(rta))->s6_addr32[0],
				   ((struct in6_addr *)RTA_DATA(rta))->s6_addr32[1],
				   ((struct in6_addr *)RTA_DATA(rta))->s6_addr32[2],
				   ((struct in6_addr *)RTA_DATA(rta))->s6_addr32[3]);
			break;
		default:
			break;
		}
...
static void nlncm_dump(const struct netconfmsg *ncm, size_t msglen)
{
	...
	rta = (const struct rtattr *)((const char *)ncm +
...
next_header:
	...
	case NLMSG_NOOP:
		break;
	case NLMSG_ERROR:
		nlmsgerr = (struct nlmsgerr *)NLMSG_DATA(nlmsg);
	...
	case RTM_DELLINK:
	case RTM_SETLINK:
		ifi = (struct ifinfomsg *)NLMSG_DATA(nlmsg);
		zlog_debug(
	...
	case RTM_NEWRULE:
	case RTM_DELRULE:
		frh = (struct fib_rule_hdr *)NLMSG_DATA(nlmsg);
		zlog_debug(
	...
	case RTM_DELNEXTHOP:
	case RTM_GETNEXTHOP:
		nhm = (struct nhmsg *)NLMSG_DATA(nlmsg);
		zlog_debug(
	...
	case RTM_NEWNETCONF:
	case RTM_DELNETCONF:
		ncm = (struct netconfmsg *)NLMSG_DATA(nlmsg);
	...
	case RTM_NEWTFILTER:
	case RTM_DELTFILTER:
		tcm = (struct tcmsg *)NLMSG_DATA(nlmsg);
		zlog_debug(
