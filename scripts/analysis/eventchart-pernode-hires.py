        ...
        for line in lines:
            ...
            if res:
                ...
                if timestamp < real_ts_base:
                    continue
                if pid in pid2gid and pid2gid[pid] <= 10:
                    gid = pid2gid[pid]
                else:
                    continue
...
parser.add_argument("-T", "--real_start_timestamp", help="pingpong end timestamp")
...
real_ts_base = float(args.real_start_timestamp)
...
ax.eventplot(
    events, orientation="horizontal", linewidth=1, linelengths=0.8, color="black"
)
...
for i in range(len(rows) - 1):
    ...
    if curr_gid != next_gid:
        # ax.axhline(y=i + 0.5, color='black', linestyle='-', linewidth=1)
        gid_ticks += [i]
        gid_labels += ["host" if curr_gid > max_gid else f"node{curr_gid}"]
...
ns_axis = ax.secondary_yaxis(location="left")
ns_axis.set_yticks(gid_ticks)
ns_axis.set_yticklabels(gid_labels)
ns_axis.tick_params(labelsize=15)
