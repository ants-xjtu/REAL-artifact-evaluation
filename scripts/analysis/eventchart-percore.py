        ...
        for line in lines:
            ...
            if res:
                ...
                if (len(comm_filter) == 0) or (command not in comm_filter):
                    ...
                    # continue
                pid = res.group(2)
                coreid = int(res.group(4))
                timestamp = float(res.group(5))
                d = {
                    "pid": pid,
                    "timestamp": timestamp,
                    "command": command,
...
print(f"args: {perf_file}, {comm_filter}, {output_file}, {plot_title}")
...
fig, ax = plt.subplots(figsize=(12, max(8, len(rows) * 0.2)))

events = [[] for _ in r_to_row]
for d in data:
    row = r_to_row[(d["command"], d["coreid"])]
    events[row].append(d["timestamp"] - base_timestamp)

ax.eventplot(events, orientation="horizontal", linewidth=1.5)
...
for i in range(len(rows) - 1):
    curr_coreid = rows[i][1]
    next_coreid = rows[i + 1][1]
    if curr_coreid != next_coreid:
        ax.axhline(y=i + 0.5, color="r", linestyle="-")
        coreid_ticks += [i]
        coreid_labels += [f"Core-{curr_coreid}"]
...
coreid_ticks += [len(rows) - 1]
coreid_labels += ["Core-0"]
...
ns_axis.set_yticks(coreid_ticks)
ns_axis.set_yticklabels(coreid_labels)
