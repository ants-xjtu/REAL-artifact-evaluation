...
def config_gen_frr(blueprint, path):
    interface_template="""auto {interface}
iface {interface} inet static
    address {network}/30
"""
    config_template = """{hostname}
# This file describes the network interfaces
{interface_config}
# ports.conf --

frr version
log file /var/log/real/frr.log debugging
...
"""
    for router in blueprint["routers"]:
        ...
        for neighbor in router["neighbors"]:
            ...
            interface_config += interface_template.format(interface=neighbor['ifname'], network=neighbor['self_ip'])

        config_text = config_template.format(
            hostname=f"node_{router['idx']}",
...
def main():
    ...
    if len(sys.argv) != 4:
        ...
        print(f"  {sys.argv[0]} frr topozoo Kdl")
        print(f"  {sys.argv[0]} frr dupzoo Fccn:2")
        return
    ...
    config_gen(image, blueprint, config_path)
    print(f"{path}")
